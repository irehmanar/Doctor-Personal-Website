...
const patientPage = async ({ req, res }) => {
  try {
    ...
    const patientsTodayCount =
      patientsToday.length > 0 ? patientsToday[0].count : 0
    const femalePatientTodayCount =
      femalePatientsToday.length > 0 ? femalePatientsToday[0].count : 0
    const malePatientTodayCount =
      malePatientsToday.length > 0 ? malePatientsToday[0].count : 0
    ...
    const femalePatients =
      femalepatients.length > 0 ? femalepatients[0].count : 0
    const foreignPatients =
      patientsByForeignlocation.length > 0
        ? patientsByForeignlocation[0].count
        : 0
    const localPatients =
      patientsByLocallocation.length > 0 ? patientsByLocallocation[0].count : 0
    res
      .status(200)
      .json({
        malePatients,
        ...
        patientsAgegt20,
        patientsAgegt40,
        femalePatientTodayCount,
        malePatientTodayCount,
        patientsTodayCount,
        newPatientsToday,
        success: true
      })
  ...
  try {
        ...
        for (let count = 0; count <= 12; count++) {
          ...
          if (!yearchange) {
            months.push({
              ...
              year: currentYear - 1,
              basicPlanCount:
                basicPlanAppointments.length > 0
                  ? basicPlanAppointments[0].count
                  : 0
            })
          } else {
            months.push({
    ...
    const findPremiumPlanByMonth = async () => {
      try {
        ...
        for (let count = 0; count <= 12; count++) {
          ...
          if (!yearchange) {
            months.push({
              ...
              year: currentYear - 1,
              premiumPlanCount:
                premiumPlanAppointments.length > 0
                  ? premiumPlanAppointments[0].count
                  : 0
            })
          } else {
            months.push({
        ...
        console.error(
          'Error finding premium plan appointments by month:',
          error
        )
        return []
      }
          ...
          if (!yearchange) {
            months.push({
              ...
              totalAppointmentCount:
                allAppointments.length > 0 ? allAppointments[0].count : 0
            })
          } else {
            months.push({
              month: monthName,
